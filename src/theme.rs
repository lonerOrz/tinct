...
mod tests {
    ...
    fn test_create_color_format() {
        let color_format = create_color_format("#ff0000").unwrap();
        assert_eq!(color_format.hex, "#ff0000");
        assert_eq!(color_format.hex_stripped, "ff0000");
        assert_eq!(color_format.red, 255);
        assert_eq!(color_format.green, 0);
        assert_eq!(color_format.blue, 0);
        assert_eq!(color_format.rgb, "rgb(255, 0, 0)");
        assert_eq!(color_format.hue as u32, 0); // Red has hue ~0
    }
    ...
    fn test_process_template() {
        ...
        let theme_value = load_theme(theme_path.to_str().unwrap()).unwrap();
        let (theme, _) = select_theme_mode(&theme_value, "dark").unwrap();
